...
from binancetrading.exchange import Exchange, _candle_list_to_df
...
class Backtest:
    ...
    def __init__(self, account: Account, exchange: Exchange, tradingbot: TradingBot, strategy: TradingStrategy, periods: int) -> None:
        ...
        self.account._set_positions(self.tradingbot.coin, self.account.paper_position, self.account.paper_cash_position)
        self.account._value_positions(self.tradingbot.symbol, init=True)
    ...
    def get_hist_data(self, symbol: str, interval: str, backtest_periods: int) -> pd.DataFrame:
        ...
        data_list = self.exchange._init_candles(symbol, interval, backtest_periods)
    ...
    def run_backtest(self, log_candles: bool = False, plot: bool = False) -> float:
        ...
        self.account._value_positions(self.tradingbot.symbol)
    ...
    def _plot_backtest_results(self, save: bool = False) -> None:
        ...
        title = f'{self.tradingbot.symbol} Backtest {str(self.strategy)} {self.backtest_periods} periods {self.tradingbot.interval} ticks'
